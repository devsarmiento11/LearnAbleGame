...
namespace TMPro.Examples
{
    ...
    public class TMP_TextSelector_A : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
    {
        ...
        void LateUpdate()
        {
            bool hasPointer = TMP_ExamplePointer.TryGetPosition(out Vector2 pointerPosition);
            m_isHoveringObject = false;

            if (hasPointer && TMP_TextUtilities.IsIntersectingRectTransform(m_TextMeshPro.rectTransform, pointerPosition, Camera.main))
            {
            ...
            if (m_isHoveringObject)
            {
                ...
                if (charIndex != -1 && charIndex != m_lastCharIndex && (TMP_ExamplePointer.ShiftPressed))
                {
